...
public:
  TokenBucketRateLimiter(long capacity, long refill_tokens,
                         std::chrono::nanoseconds refill_period)
      : capacity_(capacity),
        refill_rate_per_ns_(static_cast<double>(refill_tokens) /
                            refill_period.count()),
...
private:
  ...
  void refill() {
    ...
    if (tokens_to_add > 0) {
      current_tokens_ = std::min(static_cast<double>(capacity_),
...
private:
  ...
  std::unordered_map<std::string, std::shared_ptr<TokenBucketRateLimiter>>
      client_limiters_;
  std::mutex map_mtx_;
...
public:
  RateLimiterManager(long capacity, long refill_tokens,
                     std::chrono::nanoseconds refill_period)
      : capacity_(capacity), refill_tokens_(refill_tokens),
  ...
  bool allowRequest(const std::string &client_id, double &out_tokens) {
    ...
    {
      // We scope this lock artificially with { } so it releases immediately
      // after lookup
      std::lock_guard<std::mutex> map_lock(map_mtx_);
      auto it = client_limiters_.find(client_id);
      if (it == client_limiters_.end()) {
        limiter = std::make_shared<TokenBucketRateLimiter>(
            capacity_, refill_tokens_, refill_period_);
    ...
    // We evaluate the bucket outside the map lock so User A doesn't block User
    // B.
...
int main() {
  ...
  std::cout
      << "Capacity: " << capacity << ", Refill: " << refill_tokens
      << " token(s) per "
      << std::chrono::duration_cast<std::chrono::seconds>(refill_period).count()
      << "s" << std::endl;
  ...
  for (int i = 0; i < 20; ++i) {
    ...
    if (allowed) {
      std::cout << "[" << time_str << "] Request " << (i + 1)
                << ": Allowed (Tokens: " << std::fixed << std::setprecision(2)
                << current_tokens << ")" << std::endl;
    } else {
      std::cout << "[" << time_str << "] Request " << (i + 1)
                << ": Rate Limited (429) (Tokens: " << std::fixed
                << std::setprecision(2) << current_tokens << ")" << std::endl;
    }
